...
%>
		...
		<form action="TgatedeviceServlet" class="form-inline pull-right">
			<div class="form-group">
				<label for="fieldname">选择:</label> <select id="fieldname"
					...
					<option value="clientdevicename"
						<%=fieldname.equals("clientdevicename")?"selected":"" %>>终端设备名称</option>
					<option value="clientdeviceid"
						<%=fieldname.equals("clientdeviceid")?"selected":"" %>>终端设备编号</option>
				</select>
			</div>
			<div class="form-group">
	...
	<table class="table table-striped">
		...
		<tbody>
			...
			<% ArrayList<Tgatedevice> users=(ArrayList<Tgatedevice>)request.getAttribute("tgatedevices");
            if(users!=null)
	         {
	          	for(int i=0;i<users.size();i++)
	         	 {
	          		Tgatedevice user=users.get(i);
	            	 %>
			<tr>
				<td><%=user.getId() %></td>
				<td><%=user.getClientdevicename() %></td>
				<td><%=user.getClientdeviceid() %></td>
				<td><%=user.getClientdeviceenabled() %></td>
				<td><a
					href="TgatedeviceServlet?action=update&id=<%=user.getId()%>"><i
						class="glyphicon glyphicon-pencil"></i></a> <a
					href="javascript:void(0)"
					onclick="javascript:if(confirm('是否确认要删除？')) location.href='TgatedeviceServlet?action=delete&id=<%=user.getId()%>'"><i
...
$(function(){
        ...
        options = {
            ...
            onPageClicked: function (event, originalEvent, type, page) {
                ...
                location.href = "TgatedeviceServlet?page=" + page +"&fieldname=<%=fieldname%>&keyword=<%=keyword%>";
            }
        };
